...
const Confirmation = () => {
	const userInput =
		useRecoilValue(
			costumerAtom
		) /* <--Detta har jag lagt till ifall jag ska ha?*/
	...
	const handleInputMessage = (event, item) => {
		const orderItems = orderContent.cartItems.map((cartItem) =>
		...
		<section className="confirmation_container">
			...
			<section className="shopping-cart">
				...
				{confirmationOrderData &&
					confirmationOrderData.cartItems &&
					confirmationOrderData.cartItems.map((dish) => (
						<li
							key={dish.name}
							className="card-container order-menu"
						>
			...
			<button
				...
				onClick={() => handleDeleteOrder(orderData)}
			>
				Ångra order
			</button>
		</section>
